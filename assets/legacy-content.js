  ...
  try {
    ...
    const renderers = {
      ...
      skills() {
        ...
        const skillIcon = (item, size = 'w-7 h-7') => item.icon
          ? `<img class="${size} object-contain" src="${esc(safeUrl(item.icon))}" alt="${esc(item.name)} logo">`
          : `<span class="font-bold">${esc(initials(item.name))}</span>`;
        const overview = all.slice(0, 6).map((item, i) => `<div class="bg-surface-container-low rounded-lg p-6 glass-panel glow-card transition-all duration-300 flex justify-between items-center group"><div class="flex items-center gap-4"><div class="w-12 h-12 rounded-full bg-surface-container flex items-center justify-center text-${i%2?'secondary':'primary'} font-bold text-xl">${skillIcon(item)}</div><span class="font-body-lg text-body-lg font-bold text-on-surface">${esc(item.name)}</span></div><span class="font-label-bold text-label-bold px-3 py-1 rounded-full bg-surface-variant text-on-surface-variant border border-white/5">${esc(item.level)}%</span></div>`).join('');
        const categoryCards = groups.map((group, i) => `<div class="bg-surface-container-low rounded-xl p-card-padding glass-panel glow-card transition-all duration-300"><div class="flex items-center gap-3 mb-6"><div class="w-10 h-10 rounded-full bg-${i%3===0?'primary':i%3===1?'secondary':'tertiary'}/10 flex items-center justify-center"><span class="material-symbols-outlined text-${i%3===0?'primary':i%3===1?'secondary':'tertiary'}">code</span></div><h4 class="font-headline-md text-headline-md text-on-surface">${esc(group.category)}</h4></div><ul class="space-y-4">${(group.items || []).map((item,j) => `<li class="flex items-center justify-between gap-4 ${j < group.items.length-1 ? 'pb-4 border-b border-white/5' : ''}"><span class="font-body-md text-body-md font-bold text-white flex items-center gap-3"><span class="w-7 h-7 rounded-md bg-surface-container-high flex items-center justify-center text-[10px] text-on-surface-variant">${skillIcon(item,'w-4 h-4')}</span>${esc(item.name)}</span><div class="h-1 bg-gradient-to-r from-primary to-secondary rounded-full flex-shrink-0" style="width:${Math.max(24, Math.round((Number(item.level)||0)*.64))}px"></div></li>`).join('')}</ul></div>`).join('');
        grid.innerHTML = `<div class="lg:col-span-5 bg-surface-container-low rounded-xl p-card-padding flex flex-col items-center justify-center highlight-glow relative overflow-hidden group"><div class="absolute inset-0 bg-gradient-to-br from-primary/10 to-transparent opacity-50"></div><div class="relative z-10 text-center flex flex-col items-center gap-6"><h2 class="font-headline-md text-headline-md text-on-surface">Most Used Tech</h2><div class="w-32 h-32 rounded-2xl bg-surface-container flex items-center justify-center border border-primary/30 shadow-[0_0_40px_rgba(83,224,118,0.4)]">${skillIcon(top,'w-20 h-20')}</div><div class="mt-4"><p class="font-stats-number text-stats-number text-white">${esc(top.name)}</p><p class="font-label-bold text-label-bold text-primary mt-2 uppercase tracking-widest">${esc(top.category)}</p></div></div></div><div class="lg:col-span-7 flex flex-col gap-6"><h3 class="font-headline-md text-headline-md text-on-surface flex items-center gap-3"><span class="material-symbols-outlined text-secondary">code</span>Skills Overview</h3><div class="grid grid-cols-1 md:grid-cols-2 gap-4">${overview}</div></div><div class="lg:col-span-12 grid grid-cols-1 md:grid-cols-3 gap-gutter mt-8">${categoryCards}</div>`;
      },
