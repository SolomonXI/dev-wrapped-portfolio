(async function () {
  ...
  const templates = {
    ...
    experience: { role: 'New role', company: '', period: '', image: '', summary: '', highlights: [] },
  ...
  editor.addEventListener('click', event => {
    ...
    if (addSkill) { const group=state.skills[Number(addSkill.dataset.addSkill)]; (group.items ||= []).push({name:'New skill',level:50,icon:''}); render(); updatePreview(true); return; }
